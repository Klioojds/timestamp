/**
 * Generate the UI builder utility file (utils/ui/ui-builder.ts).
 *
 * Separates DOM creation and manipulation from the renderers.
 * Uses document.createElement() pattern (no innerHTML) for security and performance.
 *
 * Includes intensity-based pulsing ring with celebration animations:
 * - IntensityLevel type for scaling animation effects
 * - Cached value elements for performance optimization
 * - Change detection to avoid unnecessary style writes
 * - Celebration animation functions for traveling light effect
 *
...
export function generateUiBuilderTs(themeName: string): string {
  const kebabName = toKebabCase(themeName);

  return `/**
 * UI Builder - DOM creation and update utilities for the ${kebabName} theme.
 *
...
export function buildThemeDOM(container: HTMLElement): ThemeElements {
  ...
  root.className = '${kebabName}-theme';
