/**
 * Generate the time page helpers file (utils/time-page/index.ts).
 *
 * Animation and state management helpers extracted from time-page-renderer.
 * Keeps the renderer factory clean and makes helpers independently testable.
 *
 * @param themeName - Kebab-case theme name
 * @returns Generated TypeScript source code
 */
export function generateTimePageHelpersTs(_themeName: string): string {
  return `/**
 * Time Page Helpers
 *
