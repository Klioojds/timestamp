/**
 * Generate the landing page renderer file (renderers/landing-page-renderer.ts).
 *
 * Creates a LandingPageRenderer for the landing page preview.
 * The background provides ambient animation behind the theme selection card.
 * Uses document.createElement() pattern (no innerHTML) for security and performance.
 *
 * Demonstrates:
 * - Consistent imports from \@themes/shared and \@themes/shared/types
 * - DEFAULT_ANIMATION_STATE from shared utilities
 * - createResourceTracker() for automatic cleanup
 * - Animation state management via onAnimationStateChange hook
 *
...
export function generateLandingPageRendererTs(themeName: string): string {
  const pascal = toPascalCase(themeName);
  const camel = toCamelCase(themeName);

  return `/**
 * ${pascal} Theme - Landing Page Renderer
 *
 * Provides ambient background animation for landing page preview.
 * This is ${pascal}'s UNIQUE visual style - not shared with other themes.
 *
...
/**
 * Creates a ${pascal} landing page renderer.
 ...
 */
export function ${camel}LandingPageRenderer(_container: HTMLElement): LandingPageRenderer {
  ...
  return {
    mount(targetContainer: HTMLElement, context?: MountContext): void {
      ...
      wrapper.className = 'landing-theme-background--${themeName}';
